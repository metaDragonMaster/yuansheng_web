...
import Api from '@/api/nzjcuserProcess.js';
...
export default {
	...
	mounted() {
		this.getProcessPhoto()
	},
	methods: {
		async getProcessPhoto() {
			let { data } = await Api.getProcessPhoto();
			console.log("getProcessPhoto",data)
			if(!!data) {
		...
		async submit($enent) {
			this.$refs['Form'].validate(async valid => {
				if (valid) {
					...
					let res;
					// if(id>0) {
					// 	res = await Api.updateProcessPhoto(this.Form);
					// } else {
						res = await Api.postProcessPhoto(this.Form);
					// }
					if (res.code == 10200) {
						let { code, data } = await Api.postProcessInfoCheck();
