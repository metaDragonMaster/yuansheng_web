...
import Api from '@/api/nzjcuserPurchase.js';
...
export default {
	...
	mounted() {
		this.getPurchasePhoto()
	},
	methods: {
		async getPurchasePhoto() {
			let { data } = await Api.getPurchasePhoto();
			console.log("getPurchasePhoto",data)
			if(!!data) {
		...
		async submit($enent) {
			this.$refs['Form'].validate(async valid => {
				if (valid) {
					// let id = this.userInfo['userOpenInfoObject']['purchaseId'];
					let res;
					// if(id>0) {
					// 	res = await Api.updatePurchasePhoto(this.Form);
					// } else {
						res = await Api.postPurchasePhoto(this.Form);
					// }
					if (res.code == 10200) {
						let { code, data } = await Api.postPurchaseInfoCheck();
