...
export default {
	...
	mounted() {
		this.getCooperativePhoto()
	},
	methods: {
		async getCooperativePhoto() {
			let { data } = await ApiNzjcuserCooperative.getCooperativePhoto();
			console.log("getCooperativePhoto",data)
			if(!!data) {
		...
		async submit($enent) {
			this.$refs['Form'].validate(async valid => {
				if (valid) {
					...
					let res;
					// if(id>0) {
					// 	res = await ApiNzjcuserCooperative.updateCooperativePhoto(this.Form);
					// } else {
						res = await ApiNzjcuserCooperative.postCooperativePhoto(this.Form);
					// }
					if (res.code == 10200) {
						let { code, data } = await ApiNzjcuserCooperative.postCooperativeInfoCheck();
